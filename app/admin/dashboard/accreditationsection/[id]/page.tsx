...
interface FormErrors {
  sectionTitle?: string;
  authPara1?: string; authPara2?: string; authPara3?: string; authPara4?: string;
  pullQuote?: string; videoSrc?: string;
...
export default function EditAccreditationSectionPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        //   certs:   d.certs?.length   ? d.certs   : [{ ...EMPTY_CERT }],
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = { ...form };
      // await api.put(`/accreditation/update/${id}`, payload);
      ...
      <div className={styles.formCard}>
        ...
        {activeTab === "video" && (
          <>
            ...
            <div className={styles.sectionBlock}>
              ...
              {([
                { key: "immersePara1" as const, label: "Paragraph 1", hint: "About Rishikesh setting", ph: "Rishikesh, the Yoga Capital of the World, invites you to embark…", req: true },
                { key: "immersePara2" as const, label: "Paragraph 2", hint: "Breathwork, asanas, meditation — optional",          ph: "From mastering breathwork and asanas to exploring meditation…",     req: false },
        ...
        {activeTab === "recognition" && (
          <div className={styles.sectionBlock}>
            ...
            {([
              { key: "recognitionPara1" as const, label: "Paragraph 1", hint: "YCB & Yoga Alliance + registration info", ph: "At AYM Yoga School in Rishikesh, all our programs are accredited by…", req: true },
              { key: "recognitionPara2" as const, label: "Paragraph 2", hint: "Best yoga TTC pitch — optional",           ph: "If you're looking for the best Yoga TTC in Rishikesh…",             req: false },
            ]).map(({ key, label, hint, ph, req }) => (
              <div key={key} className={styles.fieldGroup}>
                <label className={styles.label}><span className={styles.labelIcon}>✦</span>{label}{req && <span className={styles.required}>*</span>}</label>
                <p className={styles.fieldHint}>{hint}</p>
                      ...
                      <div className={styles.certImageUpload}>
                        <label className={`${styles.uploadArea} ${styles.uploadAreaSm}`}>
                          ...
                          {cert.imagePreview
                            ...
                            : (<><span className={styles.uploadIcon}>🏅</span><span className={styles.uploadText}>Upload / Replace Image</span><span className={styles.uploadSubtext}>JPG, PNG, WEBP</span></>)}
                        </label>
                      </div>
