...
══════════════════════════════════════════════ */
const defaultFacilities: CampusFacility[] = [
  { bold: "Location:", text: "Our campus is a spacious blend of natural beauty and airy buildings, creating a quiet and contemplative atmosphere. The AYM campus is one of the most lush campuses among Yoga TTC schools in Rishikesh. The spacious and vast gardens maintain a peaceful ambience to learn, relax, and rejuvenate in the lap of nature. Our yoga TTC ashram in Rishikesh spans 50,000 square feet of land in the heart of Rishikesh, complemented by the distant sound of mantras being chanted from the main hall. Our Authentic Yoga Ashram in Rishikesh is located in Upper Tapovan.", image: null, imageAlt: "" },
  { bold: "AYM Buildings/Wings:", text: "We have a total of six wings, each with 20 to 25 rooms, as well as yoga studios, a library, an Ayurveda spa centre, and a swimming pool. Each is built in such a way that it gets plenty of fresh air and sunlight.", image: null, imageAlt: "" },
  { bold: "AYM Accommodation:", text: "We have a variety of accommodations at our Transformative Yoga teacher training centre in Rishikesh. Like the Luxury room, a private deluxe air-conditioned room, deluxe double room, a twin-sharing room, a dormitory for boys, and a dormitory for girls. All rooms feature windows for natural light and fresh air.", image: null, imageAlt: "" },
  { bold: "AYM Garden:", text: "At the centre of our campus, we have a beautiful garden with lush greenery, featuring large mango and avocado trees, as well as swinging chairs. Sunlight dappled through the broad leaves of the mango tree, and the flower beds overflowed with vibrant colours.", image: null, imageAlt: "" },
  { bold: "AYM Yoga studios:", text: "Our campus has 10 Yoga halls designed to help you leave the outside world behind. Soft, wooden flooring and sun-drenched, large windows fill the space with natural light, creating a calm atmosphere perfect for yoga practice.", image: null, imageAlt: "" },
  { bold: "AYM Dining space and cuisine:", text: "Our dining hall is a peaceful space where mindful eating and nourishing, simple meals are served. We emphasize providing sattvic, healthy, Indian vegetarian, locally sourced meals that support yoga practice.", image: null, imageAlt: "" },
  { bold: "AYM Central Temple or Agnihotra shala:", text: "Agnihotra shala is a space for performing the Vedic healing ritual, which purifies the atmosphere and promotes physical, mental, and spiritual well-being. In this space, we burn fire using herbal medicine, sandalwood, and chant positive Mantras.", image: null, imageAlt: "" },
  { bold: "Swimming pool:", text: "A small pool in open air and in sunlight, designed for the Healing properties of water and being used for water yoga, pregnancy yoga, teacher training classes, water breathing exercise and contemplation.", image: null, imageAlt: "" },
  { bold: "AYM Ayurveda centre:", text: "You can step away from every tension into a world of relaxation, rejuvenation, and natural healing at AYM Ayurveda Center in Rishikesh India. Our campus also offers combined yoga and ayurveda teacher training courses in Rishikesh.", image: null, imageAlt: "" },
  { bold: "Sound / Reiki healing studios:", text: "We have three sound healing studios, a haven of peace featuring soothing light and many sound healers from the Himalayas.", image: null, imageAlt: "" },
  { bold: "A yoga library:", text: "The space is a blend of traditional and modern ambience. Having books on yoga philosophy, ayurveda, computers, and high-speed internet.", image: null, imageAlt: "" },
  { bold: "The atmosphere of AYM Yoga School in Rishikesh:", text: "The vibe of the Aym campus is full of spiritual energy, with a community of residential yoga teacher training certification students, long-stay students who indulge in self-practice of yoga, and yoga volunteers, as well as individuals pursuing Ayurveda courses and treatments, or hoteliers. The campus is peaceful, with the soft sound of nature, sparrows, and a modern community, all set against a fresh mountain breeze and sunlight.", image: null, imageAlt: "" },
];
...
const defaultForm: FormData = {
  alignTitle: "", salutation: "",
  alignPara1: "", alignPara2: "", alignPara3: "",
  bodyPlanes: [
  ...
  planesPara: "",
  bodyPlanesImage: null, bodyPlanesImageAlt: "",
  outdoorImage: null,    outdoorImageAlt: "",   outdoorCaption: "",
  highlight1: "", highlight2: "",
  campusTitle: "",
  campusFacilities: defaultFacilities,
  promoCard1: { title: "", text: "", link: "" },
  promoCard2: { title: "", text: "", link: "" },
  ctaHeading: "", ctaSubtext: "", whatsappLink: "",
  masterQuote: "", masterAttrib: "",
  journeyParas: [{ text: "" }],
  namesteText: "",
};
...
/* ══════════════════════════════════════════════
   FACILITY IMAGE UPLOAD (compact, inline)
══════════════════════════════════════════════ */
    ...
    <div style={{ marginTop: "0.75rem" }}>
      ...
      {!preview ? (
        ...
        <div className={styles.imagePreviewWrap} style={{ marginBottom: "0.5rem", maxHeight: "160px" }}>
          <img src={preview} alt="Facility preview" className={styles.imagePreview} style={{ maxHeight: "140px" }} />
          <div className={styles.imagePreviewOverlay}>
...
export default function AddAYMFullPagePage() {
  ...
  /* Track which facility rows are expanded */
  ...
  const setPromo = (card: "promoCard1" | "promoCard2", field: keyof PromoCard, val: string) => {
    setForm((p) => ({ ...p, [card]: { ...p[card], [field]: val } }));
  ...
  const updatePlane = (i: number, field: keyof BodyPlaneItem, val: string) =>
    setForm((p) => {
      const arr = [...p.bodyPlanes];
      arr[i] = { ...arr[i], [field]: val };
      return { ...p, bodyPlanes: arr };
    });
  const addPlane = () => {
    if (form.bodyPlanes.length >= 8) return;
    setForm((p) => ({ ...p, bodyPlanes: [...p.bodyPlanes, { label: "", listItem: "" }] }));
  };
  const removePlane = (i: number) =>
    setForm((p) => ({ ...p, bodyPlanes: p.bodyPlanes.filter((_, idx) => idx !== i) }));

  /* ── Campus Facilities ── */
  const updateFacility = (i: number, field: keyof CampusFacility, val: string | File | null) =>
    setForm((p) => {
      const arr = [...p.campusFacilities];
      arr[i] = { ...arr[i], [field]: val };
      return { ...p, campusFacilities: arr };
    });
  const addFacility = () => {
    if (form.campusFacilities.length >= 20) return;
    const newIdx = form.campusFacilities.length;
    setForm((p) => ({
      ...p,
      campusFacilities: [...p.campusFacilities, { bold: "", text: "", image: null, imageAlt: "" }],
    }));
    setExpandedFacilities((prev) => new Set(prev).add(newIdx));
  };
  const removeFacility = (i: number) => {
    setForm((p) => ({ ...p, campusFacilities: p.campusFacilities.filter((_, idx) => idx !== i) }));
  ...
  const updateJourney = (i: number, val: string) =>
    setForm((p) => {
      const arr = [...p.journeyParas];
      arr[i] = { text: val };
      return { ...p, journeyParas: arr };
    });
  const addJourney = () => {
    if (form.journeyParas.length >= 8) return;
    setForm((p) => ({ ...p, journeyParas: [...p.journeyParas, { text: "" }] }));
  };
  const removeJourney = (i: number) =>
    setForm((p) => ({ ...p, journeyParas: p.journeyParas.filter((_, idx) => idx !== i) }));
  ...
  const validate = (): boolean => {
    ...
    if (!form.bodyPlanesImage)         e.bodyPlanesImage  = "Body planes diagram image is required";
    if (!form.outdoorImage)            e.outdoorImage     = "Outdoor group photo is required";
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        if (v instanceof File) fd.append(k, v);
          ...
          // Send facility images separately
          (v as CampusFacility[]).forEach((fac, i) => {
            if (fac.image) fd.append(`facilityImage_${i}`, fac.image);
          });
          const serialisable = (v as CampusFacility[]).map(({ image, ...rest }) => rest);
          fd.append(k, JSON.stringify(serialisable));
        } else if (Array.isArray(v) || (typeof v === "object" && v !== null))
          fd.append(k, JSON.stringify(v));
        else if (v !== null && v !== undefined)
          fd.append(k, String(v));
      });
      // await api.post("/aym-full-page/create", fd, { headers: { "Content-Type": "multipart/form-data" } });
        ...
        {activeTab === "alignment" && (
          <>
            ...
            <div className={styles.sectionBlock}>
              ...
              <div className={styles.fieldGroup}>
                <label className={styles.label}><span className={styles.labelIcon}>✦</span>
                  Section Title (H2)<span className={styles.required}>*</span>
                </label>
              ...
              <div className={styles.fieldGroup}>
                <label className={styles.label}><span className={styles.labelIcon}>✦</span>
                  Salutation<span className={styles.required}>*</span>
                </label>
                ...
                <div className={`${styles.inputWrap} ${errors.salutation ? styles.inputError : ""} ${form.salutation && !errors.salutation ? styles.inputSuccess : ""}`}>
                  <input type="text" className={styles.input}
                    ...
                    value={form.salutation} maxLength={80}
            ...
            <div className={styles.sectionBlock}>
              ...
              <ImageUploadField
                ...
                hint="Diagram showing Sagittal, Coronal & Transverse planes — displayed left of the body planes list on the frontend"
                file={form.bodyPlanesImage}
                altText={form.bodyPlanesImageAlt}
                altPlaceholder="e.g. Yoga body planes diagram - Sagittal, Coronal and Transverse planes"
                errorMsg={errors.bodyPlanesImage}
                required
            ...
            <div className={styles.sectionBlock}>
              ...
              <div className={styles.fieldGroup}>
                ...
                <div className={`${styles.inputWrap} ${errors.planesPara ? styles.inputError : ""} ${form.planesPara && !errors.planesPara ? styles.inputSuccess : ""}`}>
                  <textarea className={`${styles.input} ${styles.textarea}`}
                    placeholder="e.g. The three planes of movement in different postures of Yoga, and a Yoga teacher should have deep knowledge of them: the names are:"
                    value={form.planesPara} maxLength={300} rows={2}
                ...
                {form.bodyPlanes.map((plane, i) => (
                  <div key={i} className={styles.statRow}>
                    ...
                    <div className={styles.statFields}>
                      <div className={styles.inputWrap}>
                        <input type="text" className={styles.input}
                          placeholder="Diagram label (e.g. Sagittal plane)"
                          value={plane.label} maxLength={40}
                          onChange={(e) => updatePlane(i, "label", e.target.value)} />
                      </div>
                      <div className={styles.inputWrap}>
                        <input type="text" className={styles.input}
                          placeholder="List item (e.g. Sagittal (Longitudinal) plane.)"
                          value={plane.listItem} maxLength={60}
                          onChange={(e) => updatePlane(i, "listItem", e.target.value)} />
                      </div>
                    </div>
                    <button type="button" className={styles.removeStatBtn}
                      onClick={() => removePlane(i)}
                      disabled={form.bodyPlanes.length <= 1}>✕</button>
                  </div>
                ))}
              </div>
              {form.bodyPlanes.length < 8 && (
                <button type="button" className={styles.addStatBtn} onClick={addPlane}>
                  + Add Plane
                </button>
              )}
            </div>
            ...
            <div className={styles.sectionBlock}>
              ...
              {([
                ...
                <div key={key} className={styles.fieldGroup}>
                  ...
                  <div className={`${styles.inputWrap} ${errors[key] ? styles.inputError : ""} ${form[key] && !errors[key] ? styles.inputSuccess : ""}`}>
                    <input type="text" className={styles.input}
                      placeholder={ph} value={form[key]} maxLength={160}
                      onChange={(e) => set(key, e.target.value)} />
            ...
            <div className={styles.sectionBlock}>
              ...
              <ImageUploadField
                ...
                hint="Full-width banner image shown at the bottom of the Alignment section — outdoor yoga class photo by the Ganges"
                file={form.outdoorImage}
                altText={form.outdoorImageAlt}
                altPlaceholder="e.g. Outdoor Yoga Practice by the Ganges, Rishikesh"
                errorMsg={errors.outdoorImage}
                required
          ...
          <>
            ...
            <div className={styles.sectionBlock}>
              ...
              <p className={styles.fieldHint} style={{ marginBottom: "1rem" }}>
                Each facility item appears as a <strong>bold label</strong> + description text on the frontend. You can also attach an optional photo per facility.
              </p>
                    ...
                    <div key={i} style={{
                      ...
                      <div
                        ...
                        style={{
                          ...
                          background: isOpen ? "rgba(224,123,0,0.06)" : "transparent",
                      ...
                      >
                        ...
                        <span style={{ color: "#a07840", fontSize: "0.8rem", flexShrink: 0 }}>
                          {isOpen ? "▲" : "▼"}
                        </span>
                        <button
                          type="button"
                          ...
                          disabled={form.campusFacilities.length <= 1}
                        >✕</button>
                      </div>
                      ...
                      {isOpen && (
                        <div style={{ padding: "1rem" }}>
                          ...
                          <div className={styles.fieldGroup} style={{ marginBottom: "1rem" }}>
                            ...
                            <div className={styles.inputWrap}>
                              <input type="text" className={styles.input}
                                ...
                                value={fac.bold} maxLength={80}
                                onChange={(e) => updateFacility(i, "bold", e.target.value)} />
                          ...
                          <div className={styles.fieldGroup} style={{ marginBottom: "1rem" }}>
                            ...
                            <div className={styles.inputWrap}>
                              <textarea className={`${styles.input} ${styles.textarea}`}
                                ...
                                value={fac.text} maxLength={800} rows={4}
                          ...
                          <FacilityImageUpload
                            file={fac.image}
                            existingUrl={fac.imageUrl}
                            altText={fac.imageAlt}
              ...
              {form.campusFacilities.length < 20 && (
                <button type="button" className={styles.addStatBtn} onClick={addFacility}
                    ...
                    return (
                      <div key={field} className={styles.fieldGroup}>
                        ...
                        <div className={`${styles.inputWrap} ${errors[errKey] ? styles.inputError : ""} ${form[cardKey][field] && !errors[errKey] ? styles.inputSuccess : ""}`}>
                          {rows > 0
                            ? <textarea className={`${styles.input} ${styles.textarea}`}
                                placeholder={ph} value={form[cardKey][field]} maxLength={max} rows={rows}
                                onChange={(e) => setPromo(cardKey, field, e.target.value)} />
                            : <input type="text" className={styles.input}
                                placeholder={ph} value={form[cardKey][field]} maxLength={max}
                                onChange={(e) => setPromo(cardKey, field, e.target.value)} />}
                          <span className={rows > 0 ? `${styles.charCount} ${styles.charCountBottom}` : styles.charCount}>
                            {form[cardKey][field].length}/{max}
                          </span>
                        </div>
        ...
        {activeTab === "cta" && (
          <>
              ...
              {([
                ...
                <div key={key} className={styles.fieldGroup}>
                  ...
                  <div className={`${styles.inputWrap} ${errors[key as keyof FormErrors] ? styles.inputError : ""} ${(form[key] as string) && !errors[key as keyof FormErrors] ? styles.inputSuccess : ""}`}>
                    {rows > 0
                      ? <textarea className={`${styles.input} ${styles.textarea}`}
                          placeholder={ph} value={form[key] as string} maxLength={max} rows={rows}
                          onChange={(e) => set(key, e.target.value)} />
                      : <input type="text" className={styles.input}
                          placeholder={ph} value={form[key] as string} maxLength={max}
                          onChange={(e) => set(key, e.target.value)} />}
                    <span className={rows > 0 ? `${styles.charCount} ${styles.charCountBottom}` : styles.charCount}>
            ...
            <div className={styles.sectionBlock}>
              ...
              {([
                { key: "masterQuote"  as const, label: "Quote Text",   max: 250, rows: 2, ph: '"The beauty of Yoga is, it shows you fitness…"' },
                ...
                {form.journeyParas.map((para, i) => (
                  <div key={i} className={`${styles.statRow} ${styles.journeyRow}`}>
                    ...
                    <div className={`${styles.inputWrap} ${styles.journeyInput}`}>
                      <textarea className={`${styles.input} ${styles.textarea}`}
                        placeholder={`Journey paragraph ${i + 1}…`}
                        value={para.text} maxLength={500} rows={3}
                    ...
                    <button type="button" className={styles.removeStatBtn}
                      ...
                      disabled={form.journeyParas.length <= 1}>✕</button>
                  </div>
                ))}
              </div>
              {form.journeyParas.length < 8 && (
                <button type="button" className={styles.addStatBtn} onClick={addJourney}>
                  + Add Paragraph
                </button>
              )}
            </div>
            ...
            <div className={styles.sectionBlock}>
              ...
              <div className={styles.fieldGroup}>
                <label className={styles.label}><span className={styles.labelIcon}>✦</span>
                  Closing Line<span className={styles.required}>*</span>
                </label>
