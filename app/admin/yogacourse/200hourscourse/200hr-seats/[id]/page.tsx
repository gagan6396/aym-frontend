...
export default function BatchesEdit() {
  ...
  const { control, handleSubmit, reset, watch, formState: { isSubmitting, isLoading } } = useForm<BatchForm>({
    defaultValues: {
      date: "", usd: "", inr: "",
      roomDorm: "", roomTwin: "", roomPrivate: "",
      bookedSeats: 0, totalSeats: 50, earlyBirdNote: "",
    },
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.get(`/two-hundred-hour/batches/get/${id}`);
        ...
        reset({
          ...
          roomDorm:      b.roomDorm      || "",
          roomTwin:      b.roomTwin      || "",
          roomPrivate:   b.roomPrivate   || "",
          bookedSeats:   b.bookedSeats   ?? 0,
          totalSeats:    b.totalSeats    ?? 50,
          earlyBirdNote: b.earlyBirdNote || "",
        });
      } catch (err: any) {
        alert(err?.response?.data?.message || "Failed to load batch data.");
      }
    })();
  }, [id, reset]);
  ...
  const onSubmit = async (data: BatchForm) => {
    try {
      await api.patch(`/two-hundred-hour/batches/update/${id}`, {
        date:          data.date,
        usd:           data.usd,
        inr:           data.inr,
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.breadcrumb}>
        ...
        <span className={styles.bcSep}>›</span>
        <span className={styles.bcCurrent}>Edit Batch</span>
      </div>

      <div className={styles.pageHeader}>
        <div className={styles.editBadge}>✏️ Edit mode — changes save to this batch only</div>
        <h1 className={styles.pageTitle}>Edit Batch</h1>
        <p className={styles.pageSubtitle}>Update date, pricing and seat availability</p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className={styles.formCard}>
          ...
          <div className={styles.sectionBlock}>
            ...
            <Controller control={control} name="date" rules={{ required: "Date is required" }}
              render={({ field, fieldState }) => (
                <TXT label="Date Range" hint='e.g. "3rd Mar – 27th Mar 2026"'
                  val={field.value} err={fieldState.error?.message} onCh={field.onChange}
                  ph="e.g. 3rd Mar – 27th Mar 2026" max={60} />
              )} />

            <div className={styles.twoCol}>
              <Controller control={control} name="usd" rules={{ required: "Required" }}
                render={({ field, fieldState }) => (
                  <TXT label="FEE (USD)" hint='e.g. "749 USD"' val={field.value}
                    err={fieldState.error?.message} onCh={field.onChange} ph="749 USD" max={30} prefix="$" />
                )} />
              <Controller control={control} name="inr" rules={{ required: "Required" }}
                render={({ field, fieldState }) => (
                  <TXT label="FEE (Indian / INR)" hint='e.g. "20,999 INR"' val={field.value}
                    err={fieldState.error?.message} onCh={field.onChange} ph="20,999 INR" max={30} prefix="₹" />
                )} />
            </div>
          </div>
          ...
          <div className={styles.sectionBlock}>
            ...
            <p className={styles.fieldHint}>Shown in table: Dorm $749 | Twin $849 | Private $1099</p>

            <div className={styles.threeCol}>
              <Controller control={control} name="roomDorm" rules={{ required: "Required" }}
                render={({ field, fieldState }) => (
                  <TXT label="Dormitory" hint="" val={field.value} err={fieldState.error?.message}
                    onCh={field.onChange} ph="749" max={10} prefix="$" />
                )} />
              <Controller control={control} name="roomTwin" rules={{ required: "Required" }}
                render={({ field, fieldState }) => (
                  <TXT label="Twin Sharing" hint="" val={field.value} err={fieldState.error?.message}
                    onCh={field.onChange} ph="849" max={10} prefix="$" />
                )} />
              <Controller control={control} name="roomPrivate" rules={{ required: "Required" }}
                render={({ field, fieldState }) => (
                  <TXT label="Private Room" hint="" val={field.value} err={fieldState.error?.message}
                    onCh={field.onChange} ph="1099" max={10} prefix="$" />
                )} />
            </div>
          </div>
          ...
          <div className={styles.sectionBlock}>
            ...
            <div className={styles.twoCol}>
              <Controller control={control} name="bookedSeats"
                render={({ field, fieldState }) => (
                  <NUM label="Booked Seats" hint="Already booked count" val={field.value}
                    err={fieldState.error?.message} onCh={field.onChange} ph="0" min={0} max={500} req={false} />
                )} />
              <Controller control={control} name="totalSeats" rules={{ required: "Required" }}
                render={({ field, fieldState }) => (
                  <NUM label="Total Seats" hint="Maximum capacity" val={field.value}
                    err={fieldState.error?.message} onCh={field.onChange} ph="50" min={1} max={500} />
                )} />
            </div>
          ...
          <div className={styles.formActions}>
            <Link href="/admin/dashboard/twohundredhourpage/batches" className={styles.cancelBtn}>← Back to List</Link>
            <button type="submit" className={`${styles.submitBtn} ${isSubmitting ? styles.submitBtnLoading : ""}`} disabled={isSubmitting}>
              {isSubmitting ? <><span className={styles.spinner} /> Saving…</> : <><span>✦</span> Update Batch</>}
            </button>
          </div>
